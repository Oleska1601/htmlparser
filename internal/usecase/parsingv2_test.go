...
func TestUsecase_GetParsingDataV2(t *testing.T) {
	type args struct {
		ctx context.Context
		url string
	}
	tests := []struct {
		...
		args args
		//cache      usecase.CacheInterface
		//pgRepo     usecase.PgRepoInterface
		...
		},
		{
			...
			setupMocks: func(cache *usecaseMocks.CacheInterface, pgRepo *usecaseMocks.PgRepoInterface,
				...
				logger.On("Error", "GetParsingDataV2 u.cache.GetValue", mock.Anything).Return()
			},
		...
		},
		{
			...
			setupMocks: func(cache *usecaseMocks.CacheInterface, pgRepo *usecaseMocks.PgRepoInterface,
				...
				logger.On("Error", "GetParsingDataV2 u.pgRepo.GetParsing", mock.Anything).Return()
			},
		...
		},
		{
			...
			setupMocks: func(cache *usecaseMocks.CacheInterface, pgRepo *usecaseMocks.PgRepoInterface,
				...
				logger.On("Error", "GetParsingDataV2 u.pgRepo.AddParsing", mock.Anything).Return()
			},
		...
		},
		{
			...
			setupMocks: func(cache *usecaseMocks.CacheInterface, pgRepo *usecaseMocks.PgRepoInterface,
				...
				logger.On("Error", "GetParsingDataV2 u.cache.SetValue", mock.Anything).Return()
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, gotErr := u.GetParsingDataV2(tt.args.ctx, tt.args.url)
			if gotErr != nil {
				if !tt.wantErr {
					t.Errorf("GetParsingDataV2() failed: %v", gotErr)
				}
				return
			}
			if tt.wantErr {
				t.Fatal("GetParsingDataV2() succeeded unexpectedly")
			}
			...
			//if true {
			//	t.Errorf("GetParsingDataV2() = %v, want %v", got, tt.want)
			//}
		})
	}
}
